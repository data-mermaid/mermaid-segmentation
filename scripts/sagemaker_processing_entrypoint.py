...
ProcessingJobs are diverse (eval, inference, batch transform, mask
generation, etc.). This entrypoint reads the run YAML, dispatches on
the `processing:` block's `--task=<name>` container_arg, and routes
to the matching subroutine in mermaidseg/. Add new tasks as the team
needs them.
"""
