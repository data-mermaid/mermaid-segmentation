...
def read_csv_via_ibis(con, bucket: str, key: str) -> pd.DataFrame | None:
    ...
    CoralNet CSVs are notoriously messy, so we override several DuckDB defaults:
    - ``null_padding=True`` — ragged rows (fewer columns than header) parse cleanly.
    - ``all_varchar=True`` — prevents type-inference failures on columns with mixed types
      (e.g. "YS-D2" in a column DuckDB would otherwise infer as BIGINT).
    - ``quote='"'`` — forces double-quote recognition so commas inside quoted free-text fields
      (e.g. ``"Sheltered fringing reef, steep"``) don't split into extra columns.

    Downstream stages select and cast only the columns they need. Returns ``None`` if DuckDB
    reports the file as missing, matching :func:`read_csv_s3`'s 404 contract.
    """
    uri = f"s3://{bucket}/{key}"
    try:
        table = con.read_csv(uri, null_padding=True, all_varchar=True, quote='"')
