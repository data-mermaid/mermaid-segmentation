...
def _count_images_in_s3(client: BaseClient, bucket: str, images_prefix: str) -> int:
    paginator = client.get_paginator("list_objects_v2")
    count = 0
...
def _images_prefix_exists(client: BaseClient, bucket: str, images_prefix: str) -> bool:
    response = client.list_objects_v2(Bucket=bucket, Prefix=images_prefix, MaxKeys=1)
...
def _audit_one_source(
    ...
    try:
        record["has_images_folder"] = _images_prefix_exists(client, bucket, images_prefix)
        if record["has_images_folder"]:
            record["n_images_s3"] = _count_images_in_s3(client, bucket, images_prefix)
