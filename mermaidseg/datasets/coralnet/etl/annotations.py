...
For every source flagged ``is_complete`` in the audit DataFrame:
    ...
    6. Keep only rows with ``status == "Confirmed"``.
    7. Append a ``source_id`` column.
...
def _load_source(csv_reader: CsvReader, bucket: str, prefix: str, source_id: int) -> pd.DataFrame:
    ...
    before_confirmed = len(out)
    out = out.loc[out["status"] == "Confirmed"].reset_index(drop=True)
    if before_confirmed and len(out) < before_confirmed:
        logger.debug(
            "source %d: kept %d/%d annotation rows with Confirmed status",
            source_id,
            len(out),
            before_confirmed,
        )
