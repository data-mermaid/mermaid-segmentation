"""Catlin Seaview PyTorch dataset.

Reads Catlin Seaview point annotations from a Parquet file on S3 and emits
``(image, source_labels)`` tuples where ``source_labels`` is in the **Catlin
Seaview label-name space** — i.e. the raw ``label_name`` values found in the
per-region annotation CSVs of the Seaview Survey Photo-quadrat dataset
(Gonzalez-Rivero et al., 2019). Mapping into the MERMAID benthic-attribute
...
class CatlinSeaviewDataset(BaseCoralDataset):
    ...
    Attributes:
        annotations_path (str): Key (relative to ``source_bucket``) of the
            ...
            ``mermaidseg/datasets/catlin_seaview/download_raw_data_and_add_to_s3.py``.
        source_bucket (str): S3 bucket name containing the dataset files.
        source_s3_prefix (str): S3 prefix under which the per-region
            ``images/<region>/<image_id>.jpg`` folders live.
    ...
    Args:
        ...
        source_s3_prefix (str, optional): S3 prefix containing the
            ``images/<region>/<image_id>.jpg`` tree.
        whitelist_regions / blacklist_regions: Optional Catlin region-code
            allowlist/denylist (e.g. ``["ATL", "IND_CHA"]``).
        **base_kwargs: Forwarded to :class:`BaseCoralDataset`.
    """

    SOURCE_NAME = "catlin_seaview"

    annotations_path: str
    source_bucket: str
    source_s3_prefix: str
    s3: boto3.client
    whitelist_regions: list[str] | None
    blacklist_regions: list[str] | None
    ...
    def __init__(
        self,
        annotations_path: str = (
            "external_validation_datasets/catlin_seaview/catlin_seaview_annotations.parquet"
        ),
        source_bucket: str = "dev-datamermaid-sm-sources",
        source_s3_prefix: str = "external_validation_datasets/catlin_seaview",
        whitelist_regions: list[str] | None = None,
        blacklist_regions: list[str] | None = None,
        **base_kwargs: Any,
    ):
        if whitelist_regions is not None and blacklist_regions is not None:
            raise ValueError("Cannot specify both whitelist_regions and blacklist_regions.")
    ...
    def load_annotations(self) -> tuple[pd.DataFrame, pd.DataFrame]:
        """Load Catlin Seaview annotations from the Parquet file on S3.

        The dataset emits labels in the Catlin Seaview source-name space, so
        each row's ``source_label_name`` already holds the canonical
        per-source label string. No Catlin -> MERMAID translation happens
        ...
        if missing:
            raise ValueError(
                f"Catlin Seaview annotations parquet at {annotations_uri} is missing required "
                f"columns: {sorted(missing)}"
            )

        df_annotations = df_annotations[list(self.REQUIRED_COLUMNS)].copy()
        df_annotations["region"] = df_annotations["region"].astype(str)
        ...
        if self.whitelist_regions is not None:
            df_annotations = df_annotations[
                df_annotations["region"].isin(self.whitelist_regions)
            ].reset_index(drop=True)
        if self.blacklist_regions is not None:
            df_annotations = df_annotations[
                ~df_annotations["region"].isin(self.blacklist_regions)
            ].reset_index(drop=True)
    ...
    def _derive_df_images_from_annotations(self, df_annotations: pd.DataFrame) -> pd.DataFrame:
        return (
            df_annotations[["image_id", "region"]]
            .drop_duplicates(subset=["image_id", "region"])
            .reset_index(drop=True)
        )
