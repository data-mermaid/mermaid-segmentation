...
def label_edge_mask(target_labels: torch.Tensor, ignore_index: int = 0) -> torch.Tensor:
    """Boolean mask of pixels that differ from a 4-neighbor (ignore excluded)."""
    edge = torch.zeros_like(target_labels, dtype=torch.bool)
    edge[:, 1:, :] |= target_labels[:, 1:, :] != target_labels[:, :-1, :]
    edge[:, :-1, :] |= target_labels[:, 1:, :] != target_labels[:, :-1, :]
    edge[:, :, 1:] |= target_labels[:, :, 1:] != target_labels[:, :, :-1]
    edge[:, :, :-1] |= target_labels[:, :, 1:] != target_labels[:, :, :-1]
    edge &= target_labels != ignore_index
    return edge


class DomainWeightedFocalLoss(torch.nn.Module):
    """Masked focal loss with optional per-class (domain/class-frequency) weights.

    Operates on logits ``[B, C, H, W]`` and target labels ``[B, H, W]``. Class weights
    typically come from a parquet-derived artifact (``weight_path``). The ignore slot
    (default 0) does not contribute.
    """
...
class EdgeAwareSegmentationLoss(torch.nn.Module):
    """Wraps a base segmentation loss with an edge-pixel CE penalty.

    Weak signal on sparse point pads (``padding: 3``); intended as Ticket 2b.
    """

    def __init__(
        self,
        base_loss_type: str = "DomainWeightedFocalLoss",
        edge_weight: float = 1.0,
        ignore_index: int = 0,
        **base_kwargs: Any,
    ) -> None:
        super().__init__()
        loss_cls = globals().get(base_loss_type)
        if loss_cls is None:
            raise ValueError(f"Unknown base_loss_type '{base_loss_type}'")
        base_kwargs.setdefault("ignore_index", ignore_index)
        self.base_loss = loss_cls(**base_kwargs)
    ...
    def forward(
        ...
        base, components = self.base_loss(outputs, target_labels)
        edge = label_edge_mask(target_labels, self.ignore_index)
        if edge.any():
            per_pixel = F.cross_entropy(
                outputs,
                target_labels,
            ...
            edge_loss = per_pixel[edge].mean()
        else:
            edge_loss = outputs.sum() * 0.0

        total = base + self.edge_weight * edge_loss
        components = dict(components)
        components["edge"] = float(edge_loss.item())
        components["classification"] = float(total.item())
